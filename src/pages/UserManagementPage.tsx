...
export function UserManagementPage() {
  ...
  const { data: usersData, isLoading } = useQuery({
    ...
    queryFn: () => searchTerm 
      ? authApi.searchUsers(searchTerm) 
      : authApi.getAllUsers(0, 50, roleFilter),
  })
  ...
  const unblockMutation = useMutation({
    mutationFn: (id: number) => authApi.unblockUser(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users-list'] })
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={user.id} className="border-primary/5 hover:bg-primary/[0.02] transition-colors group">
