...
import { Search, Filter, BookOpen, AlertCircle, TrendingUp, Sparkles, SlidersHorizontal, ArrowUpDown } from 'lucide-react'
...
export function SearchPage() {
  ...
  const { data: searchResults, isLoading, error } = useQuery({
    ...
    enabled: true, // We want it to run even for empty query if needed, or we handles fallback
  })
  ...
  return (
    <div className="space-y-12 pb-24 relative overflow-hidden">
      ...
      <section className="container mx-auto px-4 space-y-8 min-h-[50vh]">
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <p className="text-sm text-muted-foreground font-medium mt-1">
                 {!isLoading && searchResults?.data?.totalElements} resources found matching your criteria.
               </p>
             </div>
          </div>
        ...
        {/* Error/Empty State */}
        {!isLoading && (searchResults?.data?.content?.length === 0 || error) && (
        ...
        {!isLoading && (searchResults?.data?.content?.length === 0 || error) && recommendations?.data && (
