...
import type { BorrowResponse } from '@/api/borrow'
import { 
  AlertTriangle, 
  Mail, 
  ArrowLeft,
...
export function OverdueManagementPage() {
  ...
  const { data: overdueData, isLoading } = useQuery({
    ...
    queryFn: () => borrowApi.getAllOverdue(0, 100),
  })

  const markAsLostMutation = useMutation({
     mutationFn: (id: number) => borrowApi.markAsLost(id),
     onSuccess: () => {
  ...
  const totalFines = overdueList.reduce((acc, curr) => acc + (curr.fine || 0), 0)
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2 border-b border-primary/5">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium text-lg leading-tight uppercase font-black text-[10px] tracking-widest opacity-60">
                      ...
                      <TableCell className="py-6 pl-8">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="space-y-0.5">
                              ...
                              <p className="text-[10px] font-black uppercase opacity-40">Account Linked: 110292</p>
                           </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                           <p className="text-xs font-black text-rose-600 flex items-center gap-2">
                              <Timer className="h-3 w-3" /> {daysOverdue} Days Delayed
                           </p>
                           <p className="text-[10px] font-bold text-muted-foreground uppercase">Expected: {new Date(item.returnDate).toLocaleDateString()}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="space-y-0.5">
                           <p className="font-black text-rose-600 text-lg">Rs. {item.fine}</p>
