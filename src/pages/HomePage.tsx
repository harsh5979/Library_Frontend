...
export function HomePage() {
  const { data: popularBooks, isLoading: isPopularLoading, error: popularError } = useQuery({
    queryKey: ['popular-books'],
    queryFn: () => booksApi.getPopular(),
  })

  console.log('Popular books:', { popularBooks, isPopularLoading, popularError });

  const { data: trendingBooks, isLoading: isTrendingLoading, error: trendingError } = useQuery({
    queryKey: ['trending-books'],
    queryFn: () => booksApi.getTrending(),
  })

  console.log('Trending books:', { trendingBooks, isTrendingLoading, trendingError });

  const { data: newArrivals, isLoading: isNewArrivalsLoading, error: newArrivalsError } = useQuery({
    queryKey: ['new-arrivals'],
    queryFn: () => booksApi.getNewArrivals(),
  })

  console.log('New arrivals:', { newArrivals, isNewArrivalsLoading, newArrivalsError });
        ...
        <div className="container px-4 sm:px-6 lg:px-8 mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-1000">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <Button variant="outline" size="lg" className="h-14 px-8 text-lg font-bold backdrop-blur-sm border-2 hover:bg-muted/50 transition-all duration-300 group">
        ...
        <div className="container px-4 sm:px-6 lg:px-8 mx-auto space-y-12 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {isTrendingLoading 
              ...
              : trendingBooks?.data.slice(0, 5).map((book) => <BookCard key={book.id} book={book} />)
            }
          </div>
        </div>
      </section>
  ...
  return (
    <section className="container px-4 sm:px-6 lg:px-8 mx-auto space-y-10">
      <div className="flex justify-between items-end">
        ...
        <Button variant="ghost" className="group text-primary hover:bg-primary/5 font-bold transition-all duration-300">
