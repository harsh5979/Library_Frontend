...
import type { User, UpdateProfileRequest } from '@/api/auth'
          ...
          <CardContent className="pt-0 relative text-center">
            ...
            <div className="mt-8 pt-8 border-t space-y-4">
              <div className="flex items-center gap-3 text-sm font-medium p-3 rounded-2xl bg-muted/20 hover:bg-muted/40 transition-colors">
                ...
                <span className="font-bold ml-auto">{user.memberSince || 'January 2024'}</span>
              </div>
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-6">
              ...
              <ProfileField 
                ...
                label="Email Address" 
                value={user.email} 
                subValue={user.isEmailVerified ? "Verified Identity" : "Unverified"}
              ...
              {user.role === 'STUDENT' && (
                <ProfileField 
                  icon={<ShieldCheck />} 
                  label="Enrollment ID" 
                  value={user.enrollmentNo || "LIBR-ST-001"} 
                />
              )}
              {user.role === 'FACULTY' && (
                <ProfileField 
                  icon={<ShieldCheck />} 
                  label="Employee ID" 
                  value={user.employeeId || "LIBR-FAC-001"} 
                />
              )}
            </div>
          </CardContent>
