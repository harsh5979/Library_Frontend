...
export function AdminDashboard() {
  ...
  const { data: popularBooksData } = useQuery({
    ...
    queryFn: () => analyticsApi.getPopularBooks(5),
  })

  const { data: recentTransactionsData } = useQuery({
    queryKey: ['recent-transactions'],
    queryFn: () => borrowApi.getAllTransactions(0, 5),
  })

  const { data: overseasData } = useQuery({
    queryKey: ['overdue-books-all'],
    queryFn: () => borrowApi.getAllOverdue(0, 5),
  })
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex gap-3">
          <Button className="h-12 px-6 font-black rounded-xl shadow-xl shadow-primary/20 bg-linear-to-r from-primary to-blue-600 border-none">
           ...
           <Tabs defaultValue="transactions" className="w-full">
            ...
            <TabsContent value="transactions" className="mt-0">
              <Card className="border-none shadow-2xl bg-background/60 backdrop-blur-xl ring-1 ring-primary/5 rounded-3xl overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {recentTransactionsData?.data?.content.map((tx) => (
                      <TableRow key={tx.id} className="border-primary/5 hover:bg-primary/[0.02] transition-colors group">
            ...
            <TabsContent value="overdue" className="mt-0">
               <Card className="border-none shadow-2xl bg-background/60 backdrop-blur-xl ring-1 ring-primary/5 rounded-3xl overflow-hidden p-6 text-center py-20">
                  ...
                  <Button variant="outline" className="font-black border-2 rounded-xl">Initialize Enforcement Flow</Button>
               </Card>
            </TabsContent>
          </Tabs>
        </div>

        {/* Right Sidebar - Trends & Categories */}
        <div className="lg:col-span-4 space-y-8">
          <Card className="border-none shadow-2xl bg-background/60 backdrop-blur-xl ring-1 ring-primary/5 rounded-3xl overflow-hidden h-fit">
            ...
            <CardContent className="space-y-4">
               {popularBooksData?.data?.map((book, idx) => (
                 <div key={idx} className="flex items-center gap-4 group cursor-pointer p-2 rounded-2xl hover:bg-muted/40 transition-all">
                    ...
                    <div className="text-right">
                       ...
                       <p className="text-[9px] font-bold text-muted-foreground uppercase">Loos</p>
                    </div>
                 </div>
               ))}
